...
fileprivate extension Stream {
  func start() -> Bool {
    ...
    guard let stream = FSEventStreamCreate(
      nil, { (_, info, count, paths, flags, _) in
        Unmanaged<Stream>.fromOpaque(info!).takeUnretainedValue()
          .handleEvents(count: count, paths: paths, flags: flags)
      }, &context, [self.url.path() as NSString] as NSArray, UInt64(kFSEventStreamEventIdSinceNow), 1.0,
  ...
  func handleEvents(count: Int, paths: UnsafeMutableRawPointer, flags: UnsafePointer<FSEventStreamEventFlags>) -> Void {
    ...
    let events = (0..<count)
      .map { i -> FileSystemEvent in FileSystemEvent(url: URL(bufferPath: pathsBuffer[i]), flag: flagsBuffer[i]) }
      .filter { $0.url.lastPathComponent != ".DS_Store" }
      .filter { "\($0.flag)" != "4259840" } /// Ignore events created by this process (I think).

    if !events.isEmpty {
      if UserInput.flag(.debug) { for event in events { HyperText.debug("\(event)") } }
      callback(events)
    }
  }
}